...
exports.getJsEntries = function () {
  var entryFiles = glob.sync('./src/modules/*/*.js')
  var map = {}
  entryFiles.forEach(filePath => {
    var subPath = filePath.substring(0, filePath.lastIndexOf('/'))
    var filename = subPath.substr(subPath.lastIndexOf('/') + 1)
...
exports.getHtmlPlugins = function () {
  var htmlFiles = glob.sync('./src/modules/*/*.html')
  var htmlPlugins = []
  htmlFiles.forEach(filePath => {
    var subPath = filePath.substring(0, filePath.lastIndexOf('/'))
    var filename = subPath.substr(subPath.lastIndexOf('/') + 1)
    var conf = {
      filename: filename + '.html',  // 文件名(包含路径)
